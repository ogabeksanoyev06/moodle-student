  ...
  <div class="task-list">
    ...
    >
      <div class="main-menu-item">
        <el-form label-position="top" :model="task" :rules="rules" ref="task" label-width="100%" class="demo-ruleForm">
        ...
        <button @click="createTask('task',$event)" class=" w-100 btn btn-primary waves-effect waves-light">
          Saqlash
        </button>
      </div>
    </Modal>
              ...
              <transition name="fade" :duration="2000">
                <tbody class="table-bordered">
                  <tr v-for="(item, index) in tasks" :key="index">
                    ...
                    <td>{{$moment(item.tasks_id.start_date).format('YYYY-MM-DD HH:mm:ss')}}</td>
                    <td>{{$moment(item.tasks_id.end_date).format('YYYY-MM-DD HH:mm:ss')}}</td>
                    <td>
  ...
  methods: {
    ...
    createTask(formName,e) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post('student/task/', formData, {
            headers: {
            ...
            this.notificationMessage("Fayl muofaqiyatli qo'shildi!", "success");
